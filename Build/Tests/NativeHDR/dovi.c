...
int main(int argc, char **argv)
{
    ...
    // Synthetic Profile 5 metadata generated by dovi_tool on the test host.
    // No picture/audio data is needed to exercise FFmpeg's RPU parser.
    FILE *file = fopen(argv[1], "rb");
    assert(file);
    uint8_t rpu[4096];
    size_t size = fread(rpu, 1, sizeof(rpu), file);
    assert(!ferror(file) && size > 2 && size < sizeof(rpu));
    fclose(file);
    ...
    puts("Native DV: strict/opt-in profile contract, original/current association, generated P5 RPU parse/flush/malformed checks passed");
}
