...
class ViewModel: ObservableObject {
    ...
    func play() {
        if !loaded {
            ...
            try! client._set(option: "vo", to: "libmpv")
...
//            check_error(mpv_set_option_string(mpv, "hwdec-image-format", "nv12")); /* color incorrect */
//            //    check_error(mpv_set_option_string(mpv, "hwdec-image-format", "yuv420p10")); /* color correct but will fallback to soft render */
//            check_error(mpv_set_option_string(mpv, "hwdec-codecs", "all"));
//            check_error(mpv_set_option_string(mpv, "gpu-hwdec-interop", "auto"));
            
//            try! client._set(option: "profile", to: "low-latency")
//            print(client._getString(property: "video-params/primaries"))
